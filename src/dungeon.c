...
void prevent_turn_overflow(void)
{
	...
	for (i = 1; i < max_towns; i++)
	{
		for (j = 0; j < MAX_STORES; j++)
		{
			...
			if (st_ptr->last_visit > -10L * TURNS_PER_TICK * STORE_TURNS)
			{
				st_ptr->last_visit -= rollback_turns;
				if (st_ptr->last_visit < -10L * TURNS_PER_TICK * STORE_TURNS) st_ptr->last_visit = -10L * TURNS_PER_TICK * STORE_TURNS;
			}
